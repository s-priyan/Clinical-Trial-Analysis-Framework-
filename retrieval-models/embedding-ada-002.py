...
for record in nli_test_data:
    ...
    for idx in range(len(primary_evidence)):
        if idx in primary_evidence_index_pred:
            data_point = {"id": uuid, "evidence" : record["primary_evidence"][idx], "label": 1}
            retrival_test_data.append(data_point)
        else:
            data_point = {"id": uuid, "evidence" : record["primary_evidence"][idx], "label": 0}
            retrival_test_data.append(data_point)
    ...
    if record["secondary_evidence"] != "":
        ...
        for idx in range(len(secondary_evidence)):
            if idx in secondary_evidence_index_pred:
                data_point = {"id": uuid, "evidence" : record["secondary_evidence"][idx], "label": 1}
                retrival_test_data.append(data_point)
            else:
                data_point = {"id": uuid, "evidence" : record["secondary_evidence"][idx], "label": 0}
                retrival_test_data.append(data_point)
        # 
    ...
    with open('../results/retrival_results/rereival_results_ada.json', 'w') as file:
